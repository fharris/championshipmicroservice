    ...
    stage('Checkout code') {
      ...
      steps {
        withKubeConfig( credentialsId: 'jenkins-token-kubernetes', serverUrl: kubernetes_proxy ) {
	          sh "kubectl apply -f appconfig/championshipms-namespace.yaml"
            ...
            sh 'kubectl -n curiosityevents create secret generic championshipms-mysql-db-secret --from-literal=SPRING_DATASOURCE_PASSWORD=$MYSQL_CREDENTIALS_PSW --from-literal=SPRING_DATASOURCE_USERNAME=$MYSQL_CREDENTIALS_USR --dry-run=client -o yaml > championshipms-mysql-db-secret.yaml'
            sh "kubectl apply -f championshipms-mysql-db-secret.yaml"
